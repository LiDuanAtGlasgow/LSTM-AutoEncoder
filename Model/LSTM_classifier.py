...
class LSTM(nn.Module):
    ...
    def label(self,lstm_out):
        x=lstm_out.view(-1,64*1024)
        x_1=self.fc1(x)
        x_2=self.fc2(x_1)
        return x_2
    
    def forward_image(self,x,device):
        image=self.image(x,device)
        return image
    def foward_label(self,x,device):
        image=self.image(x,device)
        label=self.label(image)
        return label

    def loss_label(self,output,target):
        loss_fn=nn.CrossEntropyLoss()
        return(loss_fn(output,target))
    
    def loss_image(self,output,target):
        loss_fn=nn.MSELoss(reduction='sum')(output,target)
        return (loss_fn)

...
##############################
#####Tarin Model(LSTM) LeaveOneOut Strategy########
##############################
def train(models,epochs,train_loader,test_loader,net,lr,batch_size,device,AutoEncoder_Type):
    ...
    for t,input_ in enumerate(train_loader):
        ...
        if (t+1)%(item_time+1)==1:
            print("[Train][Batch_Size:%d/%d]Duration[%f]"
                %(t+1,len(train_loader),time.time()-start_time))
            start_time=time.time()
    ...
    for t,input_ in enumerate(test_loader):
        ...
        if (t+1)%(item_time+1)==1:
            print("[Test][Batch_Size:%d/%d]Duration[%f]"
                %(t+1,len(test_loader),time.time()-start_time))
            start_time=time.time()
        samples={"picture":image,"label":label}
        test_random.append(samples)
    ...
    for i in range(len(test_random)):
        for t in range(len(test_random[i]['label'])):
            exchange_matrix.append(test_random[i]['label'][t])
    exchange_matrix_enc=encoder.fit_transform(exchange_matrix)
    for i in range(len(test_random)):
        for t in range(len(test_random[i]['label'])):
            test_random[i]['label'][t]=exchange_matrix_enc[i*len(test_random[i]['label'])+t]
            test_random[i]['label'][t]=torch.tensor([test_random[i]['label'][t]],device=device).long()
    ...
    inputs_randperm_val=inputs_random[int(len(inputs_random)*0.80):len(inputs_random)]
    ...
    ssim_stat=[]
    mse_stat=[]
    for z in range (len(test_random)):
        sample=test_random[z]
        inputs=sample['picture']
        sequ_length=1
        inputs=inputs.to(device)
        inputs=Variable(inputs)
        item_time=int(len(test_random)/10)
        for i in range(len(inputs)-3*sequ_length):
            input_pred=inputs[i:i+3*sequ_length]
            target=inputs[i+3*sequ_length:i+4*sequ_length]
            inputs_encor=net.encoder(input_pred)
            ...
            if((i+1)%(len(inputs)-4*sequ_length)==1) and ((z+1)%(item_time+1)==1):
                print("[Test][Test Time:%d/%d][Duration:%f][SSIM:%f][MSE:%d]"
                    %(z+1,len(test_random),time.time()-train_start_time,mean_stat(ssim_stat),mean_stat(mse_stat)))
                train_start_time=time.time()

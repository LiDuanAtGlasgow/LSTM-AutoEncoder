...
def train(model,epochs,data_loader,test_loader,net,lr,device,AutoEncoder_Type,Classifier):
    ...
    for z in range (len(test_random)):
        ...
        test.append(result)
        if (z+1)%(item_time+1)==1:
            print("[Test][Test Batch:%d/%d][Duration:%f]"%(z+1,len(test_random),time.time()-start_time),

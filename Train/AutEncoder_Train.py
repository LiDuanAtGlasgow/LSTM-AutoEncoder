...
def train(net,train_loader,test_loader,batch_size,epochs,learning_rate,device,AutoEncoder_Type):
    ...
    optimiser=optim.Adam(net[0].parameters(),lr=learning_rate)
    tra_los_eps=[]
    tes_los_eps=[]
    tra_all_eps=[]
    epochs=int(epochs)
    ...
    for n_epochs in range(epochs):
        ...
        for i,input_ in enumerate(train_loader):
            ...
            optimiser.zero_grad()
            recon_batch=net[0](inputs)
            loss=net[0].loss_function(recon_batch,inputs)
            loss.backward()
            optimiser.step()

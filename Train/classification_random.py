...
def train(net,data_loader,epochs,lr,device,AutoEncoder_Type,Classifier):
    ...
    net=net[0].to(device)
    scheduler=optim.lr_scheduler.StepLR(optimiser_label,step_size=4,gamma=0.1,last_epoch=-1)
    ...
    for n_epoch in range(epochs):
        ...
        total_number=len(inputs_randperm_train)*len(inputs)
        ...
        total_number=len(inputs_randperm_val)*len(inputs)
